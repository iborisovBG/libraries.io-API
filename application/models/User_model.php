...
class User_model extends CI_Model {
    ...
    public function getUserAllData($email)
    {
        ...
        {
            error_log('Няма намерен потребител getUserAllData('.$email.')');
            return false;
        }
    }
    ...
    public function updateUserInfo($post)
    {
        ...
        if(!$success){
            error_log('Не мога да обновя updateUserInfo('.$post['user_id'].')');
            return false;
        }
    ...
    public function checkLogin($post)
    {
        ...
        if($count == 1){
            ...
            {
                error_log('Неуспешен опит за логин attempt('.$post['email'].')');
                return false;
            }else{
    ...
    public function getUserInfoByEmail($email)
    {
        ...
        if($this->db->affected_rows() > 0){
            ...
            error_log('Не открит потребител getUserInfo('.$email.')');
            return false;
        }
    }
    ...
    {   
        ...
        if(!$success){
            error_log('Неуспех при обновяване на парола updatePassword('.$post['user_id'].')');
            return false;
        }        
        return true;
    } 
